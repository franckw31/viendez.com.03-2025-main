...
<body>
    ...
    <div class="container">
        ...
        <!-- Liste des activités (3 dernières et 3 prochaines) -->
        <div style="margin-bottom: 30px;">
            <label for="selectActivite" style="color: var(--color-blue); font-size: 1.2em; font-weight: bold;">Choisir une autre activité :</label>
            <form method="GET" action="fullscreen-player-simple.php" style="display:inline;">
                <select name="uid" id="selectActivite" class="joueurs-select" style="width:auto; display:inline-block;">
                    <?php
                    // Récupérer 3 dernières et 3 prochaines activités, puis trier globalement par date
                    $acts = [];
                    $last_acts = mysqli_query($con, "SELECT `id-activite`, `titre-activite`, `date_depart` FROM activite WHERE `date_depart` < NOW() ORDER BY `date_depart` DESC LIMIT 3");
                    while ($act = mysqli_fetch_assoc($last_acts)) {
                        $acts[] = $act;
                    }
                    $next_acts = mysqli_query($con, "SELECT `id-activite`, `titre-activite`, `date_depart` FROM activite WHERE `date_depart` >= NOW() ORDER BY `date_depart` ASC LIMIT 3");
                    while ($act = mysqli_fetch_assoc($next_acts)) {
                        $acts[] = $act;
                    }
                    // Tri global par date
                    ...
                    if (count($acts) > 0) {
                        foreach ($acts as $act) {
                            echo '<option value="' . $act['id-activite'] . '"' . ($act['id-activite'] == $id ? ' selected' : '') . '>' . htmlspecialchars($act['titre-activite']) . '</option>';
                        }
                    } else {
                        echo '<option value="' . $id . '" selected>Aucune autre activité disponible</option>';
                    }
                    ?>
                </select>
                <button type="submit" class="btn-confirm" style="padding: 10px 20px; margin-left: 10px;">Changer</button>
            </form>
        </div>
