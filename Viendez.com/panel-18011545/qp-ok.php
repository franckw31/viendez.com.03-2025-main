...
<div class="container">
            ...
            <table class="simple-form-table">
                <tr>
                    ...
                    <td>
                        ...
                        $three_days_ago = date('Y-m-d', strtotime('-3 days', strtotime($actu2)));
    ...
    <div class="card">
        ...
        <form method="post">
            <table class="simple-form-table">
                 ...
                 <tr>
                    ...
                    <td>
                        <?php
                        // (PHP code to populate select - remains the same, ADD ID)
                        ...
                        if ($acti_reg) {
                            ...
                            while ($choix = mysqli_fetch_assoc($acti_reg)) {
                                $selected_attr = ($choix["id-activite"] == $current_selected_activity) ? ' selected' : '';
                                $formatted_date = date("d/m/Y H:i", strtotime($choix["date_depart"]));
                                echo "<option value='" . htmlspecialchars($choix["id-activite"]) . "'$selected_attr>" . $formatted_date . " (" . htmlspecialchars($choix['titre-activite']) . ")</option>"; // Added title for clarity
                            }
        ...
        <script>
            ...
            const activitySelect = document.getElementById('acti_reg_select'); // Assuming the filter form's select is used or has this ID
            ...
            // Initial call in case a table is pre-selected (though unlikely with current setup)
            // updateSiegeOptions(); // Might not be needed if table defaults to empty
            ...
            if (tableSelect) {
             ...
             // Optional: Add listener to activity select if you want instant feedback
             // without relying solely on the main "Filtrer" button causing a page reload.
             // This would require an AJAX call to fetch new occupiedSeats data.
             /*
             if (activitySelect) {
                 activitySelect.addEventListener('change', function() {
                     // 1. Get selected activity ID: this.value
                     // 2. Make AJAX request to a PHP endpoint (e.g., get_available_seats.php?acti=...)
                     // 3. On success:
                     //    - Update the global `occupiedSeats` variable with the response JSON
                     //    - Reset tableSelect to '-- Choisir Table --'
                     //    - Reset and disable siegeSelect
                     //    - Enable tableSelect
                     //    - Enable the submit button
                     // 4. Handle errors
                     console.log("Activity changed to: ", this.value);
                     // Placeholder: alert("Activity changed. Seat availability needs refresh (AJAX not implemented).");
                     // For now, user needs to use the main 'Filtrer' button to reload occupied seats.
                 });
             }
             */
                            ...
                            if (mysqli_stmt_execute($stmt_list)) {
                                ...
                                if (mysqli_num_rows($result) > 0) {
                                    ...
                                    while($row = mysqli_fetch_assoc($result)) {
                                        ...
                                        echo "<td class='cell-center'><input type='number' name='participations[$index][table]' value='" . htmlspecialchars((string)$row['id-table']) . "' min='1' max='9' " . ($selected_activity ? '' : 'readonly') . "></td>"; // Max changed to 9
                                        echo "<td class='cell-center'><input type='number' name='participations[$index][siege]' value='" . htmlspecialchars((string)$row['id-siege']) . "' min='1' max='10' " . ($selected_activity ? '' : 'readonly') . "></td>"; // Max remains 10
                                        ...
                                        echo "<td class='cell-right'><input type='number' name='participations[$index][recave]' value='" . htmlspecialchars((string)$row['recave']) . "' step='1' min='0' max='4' placeholder='0' " . ($selected_activity ? '' : 'readonly') . "></td>"; // Max added 4
        ...
        <form method="post">
            <table class="simple-form-table">
                <tr>
                    ...
                    <td>
                        <input class="form-control" type="number" id="nb_joueurs_payes" name="nb_joueurs_payes" value="<?php echo isset($_POST['nb_joueurs_payes']) ? htmlspecialchars($_POST['nb_joueurs_payes']) : round($total_participants * 0.25); ?>" min="1" max="<?php echo $total_participants; ?>">
                    </td>
                </tr>
                <tr>
                    <th></th>
                    <td class="btn-container-cell">
                        <div class="btn-container">
                            <button type="submit" class="btn btn-primary-orange2" name="submit_distribution">Calculer la distribution</button>
                        </div>
                    </td>
                </tr>
            </table>
        ...
        if (isset($_POST['submit_distribution']) && $selected_activity !== null && $data_found) {
            ...
            if ($nb_joueurs_payes > 0 && $nb_joueurs_payes <= $total_participants) {
                ...
                switch ($nb_joueurs_payes) {
                    case 2:
                        $distributions = array(0.60, 0.40);
                        break;
                    case 3:
                        $distributions = array(0.50, 0.30, 0.20);
                        break;
                    case 4:
                        $distributions = array(0.45, 0.30, 0.15, 0.10);
                        break;
                    case 5:
                        $distributions = array(0.40, 0.27, 0.15, 0.10, 0.08);
                        break;
                    case 6:
                        $distributions = array(0.35, 0.25, 0.15, 0.11, 0.08, 0.06);
                        break;
                    case 7:
                        $distributions = array(0.33, 0.23, 0.15, 0.11, 0.08, 0.06, 0.04);
                        break;
                    case 8:
                        $distributions = array(0.33, 0.23, 0.14, 0.10, 0.07, 0.05, 0.04, 0.04);
                        break;
                    default:
                        echo "<div class='alert alert-warning'>Nombre de joueurs payés non pris en charge.</div>";
                        break;
                }

                if (!empty($distributions)) {
                    $total_distribution = array_sum($distributions);
                    if (abs($total_distribution - 1) > 0.001) {
                        echo "<div class='alert alert-danger'>La somme des pourcentages de distribution ne correspond pas à 100%.</div>";
                    } else {
                        echo "<table class='data-table'>";
                        echo "<thead><tr><th>Position</th><th>Pourcentage</th><th>Gain Estimé</th></tr></thead>";
                        echo "<tbody>";
                        for ($i = 0; $i < count($distributions); $i++) {
                            $pourcentage = $distributions[$i] * 100;
                            $gain_estime_raw = $price_pool_display * $distributions[$i];
                            $gain_estime_rounded = round($gain_estime_raw / 5) * 5; // Round to nearest 5
                            echo "<tr><td>" . ($i + 1) . "</td><td>" . number_format($pourcentage, 2) . "%</td><td>" . number_format($gain_estime_rounded, 2) . "</td></tr>";
                        }
                        echo "</tbody>";
                        echo "</table>";
                    }
            ...
            echo "<div class='alert alert-warning'>Veuillez sélectionner une activité pour calculer la distribution.</div>";
        } elseif (isset($_POST['submit_distribution']) && !$data_found) {
            echo "<div class='alert alert-warning'>Aucun participant trouvé pour l'activité sélectionnée.</div>";
        }
        ?>
    </div>
