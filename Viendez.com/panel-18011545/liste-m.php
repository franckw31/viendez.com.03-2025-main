...
if (strlen($_SESSION['id']) == 0) {
    ...
    function fetchMembres() {
     ...
     //   $result = mysqli_query($conn, "SELECT 'id-membre', pseudo, droits, fname, lname, type, posting_date, lastip, nbpoints, association_date, naissance_date, CodeV, verification, telephone, email, photo, commentaire, rue, ville, country, longitude, latitude, icon, 'ico-siz', lien, 'lien-id', 'lien-texte', 'lien-texte-fin', defstr, def_nbj, def_buy, def_rak, def_bou, def_rec, def_jet, def_bon, def_add, def_ant FROM membres");
        $result = mysqli_query($conn, "SELECT * FROM `membres` WHERE `id-membre` > 0");
