...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['players'])) {
    ...
    try {
        // Supprimer seulement les anciennes entrées pour cette session
        $pdo->exec("DELETE FROM poker_players");
        
        $stmt = $pdo->prepare("INSERT INTO poker_players (name, buyin) VALUES (?, ?)");
        $_SESSION['players'] = [];
...
// Charger les joueurs depuis la base si la session est vide
if (empty($_SESSION['players'])) {
    $stmt = $pdo->query("SELECT name, buyin FROM poker_players");
    $_SESSION['players'] = $stmt->fetchAll(PDO::FETCH_ASSOC);
}
...
if (isset($_GET['reset'])) {
    session_destroy();
    // Supprimer les joueurs de la base
    $pdo->exec("DELETE FROM poker_players");
            ...
            <form method="POST" id="players-form">
                <div class="form-group">
                    ...
                    <select id="saved-player">
                        ...
                        <?php 
                        $savedPlayers = $pdo->query("SELECT DISTINCT name, buyin FROM poker_players ORDER BY name");
