...
if (isset($_POST['generate'])) {
    ...
    $idm = mysqli_real_escape_string($conx, $_POST['pseudo']);
    ...
    if (!empty($pseudo)) {
        ...
        $res_member = mysqli_query($conx, "SELECT password_ext,id_membre FROM membres WHERE pseudo = '$pseudo'");
        if ($member = mysqli_fetch_assoc($res_member)) {
            $password = $member['password_ext'];
            $idm = $member['id_membre'];
            $content = "http://$host/panel/quickview.php?pseudo=" . urlencode($pseudo) . "&idm=" .urlencode($idm) . "&passwd=" . urlencode($password);
