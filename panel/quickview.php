...
if ($pseudo_get && $pass_get) {
	$q_auth = mysqli_query($con, "SELECT `id-membre`, `pseudo` FROM membres WHERE pseudo = '$pseudo_get' AND password = '$pass_get'");
		...
		if ($user_id > 0 && $act_id > 0) {
			...
			if(isset($_POST['status']) || isset($_POST['id_rake'])) {
				...
				if($exists) {
					...
					mysqli_query($con, "INSERT INTO participation (`id-membre`, `id-activite`, `nom-membre`, `option`, `id-challenge`, `ordre`, `valide`, `classement`, `id_rake`, `ds`) 
										VALUES ('$user_id', '$act_id', '$m_name', '$final_status', '$challenge_id', '$next_ordre', 'Actif', '1', '$final_rake', NOW())");
				}
			}
		}
			...
			<div class="app-content">
				...
				<div class="main-content">
					<div class="wrap-content container" id="container">
						...
						<div class="row">
							<div class="col-sm-4">
								<a href="voir-blindes.php?uid=<?php echo $id; ?>" class="dashboard-card">
							...
							<div class="col-sm-4">
								<a href="liste-participants-activite.php?id=<?php echo $id; ?>" class="dashboard-card card-purple">
							...
							<div class="col-sm-4">
								<a href="voir-membre.php?id=<?php echo $row_act['id-membre']; ?>" class="dashboard-card">
									<div class="card-icon"><i class="fa fa-map-marker"></i></div>
									<div class="card-title">Organisateur & Lieu</div>
									<div class="card-stat" style="font-size: 16px; font-weight: normal;">
										<?php 
										$id_orga = $row_act['id-membre'];
										$q_orga = mysqli_query($con, "SELECT pseudo FROM membres WHERE `id-membre` = '$id_orga' LIMIT 1");
										$r_orga = mysqli_fetch_array($q_orga);
										echo "<strong>Organisateur:</strong> " . htmlentities($r_orga['pseudo']) . "<br>";
										echo "<strong>Lieu:</strong> " . htmlentities($row_act['rue']) . ", " . htmlentities($row_act['ville']);
										?>
									</div>
								</a>
							</div>
