...
function redistribuerPrizePool($players, $ranking) {
    ...
    // Créer un tableau des joueurs classés avec leur buy-in
    $rankedPlayers = [];
    $uniqueBuyins = [];
    $totalPrizePool = 0;
    
    // Récupérer les 5 premiers joueurs et leurs buy-ins
    for ($i = 0; $i < 5; $i++) {
        ...
        foreach ($players as $player) {
            if ($player['name'] === $playerName) {
                ...
                $totalPrizePool += floatval($player['buyin']);
                $playerFound = true;
                break;
            }
        }
    ...
    // Trier les buy-ins par ordre décroissant
    rsort($uniqueBuyins);
    
    // Calculer les prize pools intermédiaires
    $prizePools = [];
    $paiements = array_fill(0, 5, 0);
    $distribue = 0;

    // Distribution initiale basée sur les niveaux de buy-in
    foreach ($uniqueBuyins as $buyin) {
        $poolSize = 0;
        $qualifiedPlayers = [];
        
        foreach ($rankedPlayers as $player) {
            if ($player['buyin'] >= $buyin) {
                $poolSize += $buyin;
        ...
        if ($poolSize > 0) {
            ...
            for ($i = 0; $i < count($qualifiedPlayers) && $i < 5; $i++) {
                $gain = round($poolSize * $repartition[$i], 2);
                $playerIndex = array_search($qualifiedPlayers[$i]['name'], array_column($rankedPlayers, 'name'));
                $paiements[$playerIndex] += $gain;
                $distribue += $gain;
            }
        }
    }

    // Ajuster pour distribuer le reste
    $reste = $totalPrizePool - $distribue;
    if ($reste > 0) {
        // Distribuer le reste proportionnellement aux 5 premiers
        $repartition = [0.37, 0.23, 0.18, 0.12, 0.10];
        for ($i = 0; $i < 5; $i++) {
            $ajustement = round($reste * $repartition[$i], 2);
            $paiements[$i] += $ajustement;
        }
    }
