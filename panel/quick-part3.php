    ...
    if ($update_success_overall && $selected_activity_id_for_update) {
        ...
        if ($stmt_update) {
            ...
            if (isset($_POST['participations']) && is_array($_POST['participations'])) {
                /* Calculate the number of paid players (25% of total participants) */
                $total_participants = count($_POST['participations']);
                $number_of_paid_players = round($total_participants * 0.25); // Round to nearest whole number
                /* The number of paid players is: $number_of_paid_players */
