...
if (isset($_POST['update_participation'])) {
    ...
    try {
        ...
        $challenge_id_query = "SELECT id_challenge FROM activite WHERE id-activite = ?";
        $stmt_challenge = mysqli_prepare($con, $challenge_id_query);
        if ($stmt_challenge) {
            mysqli_stmt_bind_param($stmt_challenge, "i", $_SESSION['selected_activity']);
            mysqli_stmt_execute($stmt_challenge);
            $challenge_result = mysqli_stmt_get_result($stmt_challenge);
            if ($challenge_row = mysqli_fetch_assoc($challenge_result)) {
                $challenge_id = intval($challenge_row['id_challenge']);
                
                // Now update gain_cumul for all participations in this challenge
                $update_gain_cumul = "
                    ...
                    WHERE p1.`id-activite` IN (
                        SELECT a.`id-activite` 
                        FROM activite a 
                        WHERE a.`id-challenge` = ?
                    )";

                $stmt_update_gain = mysqli_prepare($con, $update_gain_cumul);
                if ($stmt_update_gain) {
                    mysqli_stmt_bind_param($stmt_update_gain, "ii", $challenge_id, $challenge_id);
                    if (!mysqli_stmt_execute($stmt_update_gain)) {
                        throw new Exception("Erreur lors de la mise à jour du gain cumulé: " . mysqli_stmt_error($stmt_update_gain));
                    }
                    mysqli_stmt_close($stmt_update_gain);
                }
            } else {
                throw new Exception("Impossible de récupérer l'ID du challenge pour l'activité sélectionnée.");
            }
            mysqli_free_result($challenge_result);
            mysqli_stmt_close($stmt_challenge);
        }

        $_SESSION['feedback'] = "<div class='alert alert-success'>Participations et gains cumulés mis à jour avec succès.</div>";
    } catch (Exception $e) {
            ...
            if ($stmt_podium = mysqli_prepare($con, $podium_query)) {
                ...
                if (mysqli_stmt_execute($stmt_podium)) {
                    ...
                    if (mysqli_num_rows($podium_result) > 0) {
                        ...
                        echo "<th class='cell-center sortable'>Table</th>";
                        echo "<th class='cell-center sortable'>Siège</th>";
                        echo "<th class='cell-center sortable'>Challenge</th>";
                        echo "<th class='cell-center sortable'>TF</th>";
                        ...
                        while ($player = mysqli_fetch_assoc($podium_result)) {
                            ...
                            echo "<td class='cell-center'>" . htmlspecialchars($player['id-table']) . "</td>";
                            echo "<td class='cell-center'>" . htmlspecialchars($player['id-siege']) . "</td>";
                            ...
                            echo "<td class='cell-right'>" . number_format(floatval($player['gain_cumul']), 2, ',', ' ') . " €</td>"; // Nouvelle colonne
...
function initAlphaKeyboard() {
 ...
 "info": false,
        ...
        "columnDefs": [
            ...
            { 
                "targets": [2, 3],  // Colonnes Table et Siège
                "type": "num",
                "orderable": true
            },
            { 
                "targets": [4, 5],  // Colonnes Challenge et TF
                "orderable": true
            },
            { 
                "targets": [6],     // Colonne Points
                "type": "num",
                "orderable": true
            },
            { 
                "targets": [7],     // Colonne Commentaire
                "type": "string",
                "orderable": true
            }
        ],
