...
$error = "";
if (isset($_GET['Reset'])) {
    if (mysqli_num_rows(mysqli_query($conx, "SELECT * FROM membres WHERE CodeV='{$_GET['Reset']}'")) > 0) {
        if (isset($_POST['submit'])) {
            ...
            if ($Pass === $Confirme_Pass) {
                $sql = "UPDATE membres SET Password ='" . $Pass . "' WHERE CodeV='{$_GET['Reset']}'";
    ...
    header("Location: /reg/forget.php");
}
